...
double Order::CalculateTotalCost() const {
  ...
  if (request_flavour_ == "Red Velvet" || request_flavour_ == "Lemon")
    cost += 5.0;
  if (request_filling_ == "Chocolate Ganache")
    cost += 8.0;
  else if (request_filling_ != "Strawberry Jam")
    cost += 5.0;
  if (request_frosting_ == "Cream Cheese Frosting")
    cost += 7.0;
  else
    cost += 5.0;
  if (request_sprinkles_ == "Gold Flakes")
    cost += 10.0;
  else if (request_sprinkles_ != "None")
    cost += 3.0;
