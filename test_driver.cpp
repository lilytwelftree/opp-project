...
public:
    ...
    void testSubmitCake() {
        ...
        // testing valid complete cake (choc sprinkles, strawberry filling, vanilla frosting)
        Cake cake1;
        ...
        bool test1 = !cake1.GetFlavour().empty() && 
                    !cake1.GetFilling().empty() && 
                    !cake1.GetTopping().empty();
        logTest("valid complete cake (chocolate sprinkles, strawberry filling, vanilla frosting)", 
        ...
        bool test2 = !cake2.GetFlavour().empty() && 
                    !cake2.GetFilling().empty() && 
                    !cake2.GetTopping().empty();
        logTest("valid complete cake (pink sprinkles, jam filling, vanilla frosting)", 
        ...
        bool test3 = !cake3.GetFlavour().empty() && 
                    !cake3.GetTopping().empty() &&
                    cake3.GetFilling().empty(); // no filling
        logTest("incomplete input (sprinkles, chocolate frosting)", 
                test3, test3 ? "" : "incomplete cake not handled properly");
        
        // testing empty cake (no flavour, empty decorations)
        Cake cake4;
        bool test4 = cake4.GetFlavour() == "None" && 
                    cake4.GetFilling().empty() && 
                    cake4.GetTopping().empty();
        logTest("empty cake (no flavour, empty decorations)", 
                test4, test4 ? "" : "empty cake not handled properly");
    }
    ...
    void testSetTime() {
        ...
        logTest("valid input level 1", test1, 
                test1 ? "" : "expected 180 seconds, got " + to_string(time1.GetLimitSeconds()));
        ...
        logTest("valid input level 3", test2, 
                test2 ? "" : "expected 150 seconds, got " + to_string(time2.GetLimitSeconds()));
        ...
        logTest("invalid level 0", test3, 
                test3 ? "" : "expected 180 seconds (default), got " + to_string(time3.GetLimitSeconds()));
        ...
        logTest("invalid level -2", test4, 
                test4 ? "" : "expected 180 seconds (default), got " + to_string(time4.GetLimitSeconds()));
        ...
        logTest("invalid level 12", test5, 
                test5 ? "" : "expected >= 40 seconds, got " + to_string(time5.GetLimitSeconds()));
    }
    ...
    void testUpdateLevel() {
        ...
        // note: since we dont have store class fully implemented, we'll test what we can
        
        Store store;
        
        // test case 1: valid input (revenue: $100, required customer satisfaction: 4)
        store.UpdateRevenue(100.0);
        store.UpdateRating(4);
        // note: we cant fully test upgradestore() without the complete implementation
        ...
        // test case 2: valid input (revenue: $99, required customer satisfaction: 4)
        ...
        // test case 3: invalid input (revenue: $-10, required customer satisfaction: 3)
        ...
        logTest("invalid input (revenue: $-10, rating: 3)", test3, 
                test3 ? "negative revenue accepted (potential bug)" : "revenue not set correctly");
    ...
    void testApplyDecoration() {
        ...
        // test case 2: valid input (frosting class, toppingchoice: "pink buttercream")
    ...
    void testSprinkles() {
        ...
        logTest("valid input (sprinkles: none)", test2, 
    ...
    void testGiveReview() {
        ...
        bool test2 = stars2 >= 3 && stars2 <= 4; // should be high but penalized for time
        logTest("perfect accuracy, zero time remaining (4/4 correct, 0s left)", test2, 
                test2 ? "" : "expected 3-4 stars, got " + to_string(stars2));
        
        // test case 3: low accuracy, good time remaining
        int stars3 = customer.GiveReview(1, 4, 120); // 1/4 correct, 120 seconds left
        ...
        bool test4 = stars4 >= 1 && stars4 <= 3; // should be low due to both factors
        ...
        logTest("invalid input - negative time (4/4 correct, -10s)", test5, 
                test5 ? "" : "negative time not handled properly, got " + to_string(stars5));
        ...
        logTest("division by zero protection (0/0 correct)", test6, 
                test6 ? "" : "expected 1 star (division by zero protection), got " + to_string(stars6));
    }
    ...
    void runAllTests() {
        ...
        cout << "Success Rate: " << (testsPassed * 100 / (testsPassed + testsFailed)) << "%" << endl;
        
        if (testsFailed == 0) {
            cout << "\nALL TESTS PASSED! Your classes are working correctly!" << endl;
        } else {
            cout << "\nSome tests failed. Check the details above for issues to fix." << endl;
        }
    }
};
