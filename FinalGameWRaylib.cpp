...
class RaylibSimpleGame {
 ...
 public:
   ...
   */
  void Cleanup() { intro_screen.Cleanup(); }
  ...
  void HandleInput() {
    ...
    string* current_component =
        nullptr;  // Points to which part of the cake we're building
  ...
  void DrawCakePreview() {
    ...
    if (game.current_flavour == "Vanilla")
      baseColor = {255, 248, 220, 255};
    else if (game.current_flavour == "Red Velvet")
      baseColor = {139, 69, 19, 255};
    else if (game.current_flavour == "Lemon")
      baseColor = {255, 255, 224, 255};
    ...
    if (game.current_sprinkles != "None") {
      ...
      for (int i = 0; i < 15; i++) {
        ...
        int x =
            cx + cos(angle) * (30 + (i % 3) * 10);  // Vary distance from center
  ...
  void DrawShop() {
    ...
    for (size_t i = 0; i < items.size(); i++) {
      ...
      Color btnColor = (game.money >= price) ? (Color){200, 255, 200, 255}
  ...
  void DrawResults() {
    ...
    if (game.time_left == 0) {
      ...
      string reaction = (matches >= 3)   ? "excellent!"
                        : (matches >= 2) ? "okay"
                                         : "poor";
      Color reactionColor = (matches >= 3)   ? GREEN
                            : (matches >= 2) ? ORANGE
                                             : RED;
