...
bool Decorations::AddDecorations() {
  ...
  if (no_of_decorations_ < kMaxDecorations) {
    ...
    cout << "Added decoration #" << no_of_decorations_ << " : "
         << decorationsList.back() << endl;

    return true;
  }
  // Exception handling: run-time
  else if (no_of_decorations_ > kMaxDecorations) {
    throw std::runtime_error("Error: Incorrect array size.");
  }
  // If full
  else {
    // Display output full
...
int Decorations::GetNoOfDecorations() const { return no_of_decorations_; }
...
void Decorations::ShowDecorationStatus() const {
  ...
  if (!decorationsList.empty()) {
    ...
    // Use loop to print each decoration, using static cast to convert unsigined
    // int from size()
